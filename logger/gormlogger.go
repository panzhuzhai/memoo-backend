...
func (log *logger) Trace(_ context.Context, begin time.Time, fc func() (string, int64), err error) {
	...
	case err != nil:
		sql, _ := fc()
		log2.Println("exec sql err:", err.Error(), "sql:", sql)
		//log.Zap.Errorf(errorFormat, gormUtils.FileWithLineNum(), err, sql)
	case elapsed > slowThreshold*time.Millisecond && slowThreshold*time.Millisecond != 0:
		sql, _ := fc()
		slowLog := fmt.Sprintf("SLOW SQL >= %v", slowThreshold)
		log.Zap.Warnf(errorFormat, gormUtils.FileWithLineNum(), slowLog, sql)
	default:
		sql, _ := fc()
		log.Zap.Debugf(sqlFormat, sql)
	}
}
