...
var (
	...
	ErrMissingLoginValues = errors.New("missing signature or address")

	// ErrFailedAuthentication indicates authentication failed, could be faulty username or password
	ErrFailedAuthentication = errors.New("incorrect signature or address")

	// ErrFailedTokenCreation indicates JWT Token failed to create, reason unknown
	ErrFailedTokenCreation = errors.New("failed to create JWT Token")

	// ErrExpiredSign indicates JWT token has expired. Can't refresh.
	ErrExpiredSign = errors.New("signature is expired") // in practice, this is generated from the jwt library not by us
	...
	// ErrInvalidSignature indicates signing algorithm is invalid, needs to be HS256, HS384, HS512, RS256, RS384 or RS512
...
func GetAddressWithResponse(c *gin.Context) string {
	address := GetAddress(c)
	if address == "" {
		serializer.WrapErrReturn(c, "args is err", nil)
	}
	return address
}

func (mw *GinJWTMiddleware) GetUserAddress(c *gin.Context) {
	address := GetAddressWithResponse(c)
	c.JSON(http.StatusOK, gin.H{
		"code":    http.StatusOK,
		"address": address,
	})
...
func GetChanel(c *gin.Context) string {
	return c.Request.Header.Get("channel")
}

func GetProject(c *gin.Context) string {
	return c.Request.Header.Get("Rune-Project")
}
